function [ETc, Dr, Ke, Ks, AW, Tc] = dualcropcov2(weather,refET,Kcb,Kcmin,h,Zr,ptab,soil,rescov,theta, faw, fc_custom)
...
for i = 1
    ...
    if Dr0 > RAW(i)
        ...
        %end
        %{
        Ks(i,1) = faw(i);%(TAW(i)-Dr(i-1))/[TAW(i)-RAW(i)]; %Eq. 84, FAO-56, transpiration reduction factor
        if faw(i) > 0.6;
            Ks(i) = 1;
        end
        if Ks(i) > 1;
    ...
    Dr(i) = Dr0; %root zone depletion at the end of the day (mm)
...
for i = 2:length(days)
    ...
    %Dr = faw(1:i); %ESK 2/13/2019
    Dr(i,1) = (1-faw(i))*TAW(i);
