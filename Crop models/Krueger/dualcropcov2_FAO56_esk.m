function [ETc, Dr, Ke, Ks, AW, Tc, Kcmax] = dualcropcov2(weather,refET,Kcb,Kcmin,h,Zr,ptab,soil,rescov,theta,fc_custom)
%DUALCROPCO calculates coefficients needed for the FAO-56 dual crop
%coefficient method
%
%TEO
%
%ESK 9/13/2018:  Added calclated crop transpiration (Tc) and added as
%returned variable.
%
% BMW 10/1/2019: Added Kcmax as returned variable
%
...
%thetai = field capacity, wilting point, and initial water content values 
%          for surface layer and entire subsoil.
...
for i = 1
    ...
    Dr0 = 0; %ESK, added 11/20/2019 to indicate zero depletion on the first day
    if Dr0 > RAW(i)
    ...
    AW(i) = TAW(i) - Dr(i); % Available water in the root zone (0 to Zr(i), mm). *Added by BMW 9/15/2016
    FAW(i) = AW(i)/TAW(i);
end
