function [ETc, Dr, Ke, Ks, AW, DP, Tc, TAW, Dr0, RAW] = dualcropcov2_forest(weather,refET,Kcb,Kcmin,h,Zr,ptab,soil,rescov,theta)
%DUALCROPCO calculates coefficients needed for the FAO-56 dual crop
%coefficient method
%
% TEO
% Edited by BMW May 2016
%
...
TEW = 1000*(theta_fc(1)-0.5*theta_wp(1))*Ze; %Eq. 73, FAO-56 total evaporable water (mm)
...
fw = 1.0; %fraction of surface wetted by precipitation
...
for i = 1
    Kcmax(i) = max((1.2 + [0.04*(u2(i)-2)-0.004*(RHmin(i)-45)]*(h(i)/3)^0.3),(Kcb(i) + 0.15)); %Eq. 72, FAO-56
    %Kcmax(i) = Kcb(i) + 0.15; %because forest height is too high, max of ~1.3, Eq. 72 FAO-56
    ...
    AW(i) = TAW(i) - Dr(i); % Available water in the root zone (0 to Zr(i)). *Added by BMW 7/6/2016
    FAW(i) = AW(i)/TAW(i);
end
...
for i = 2:length(days)
    Kcmax(i) = max((1.2 + [0.04*(u2(i)-2)-0.004*(RHmin(i)-45)]*(h(i)/3)^0.3),(Kcb(i) + 0.15)); %Eq. 72, FAO-56
    %Kcmax(i) = Kcb(i) + 0.15; %because forest height is too high
    fc(i) = ((Kcb(i)-Kcmin)/(Kcmax(i)-Kcmin))^(1+0.5*h(i)); %approximate crop ground cover, Eq. 76, FAO-56
    few(i) = min(1-fc(i),fw); %fraction of surface exposed and wetted
    if De(i-1) > REW
    ...
    p = min([p 0.8]); %Fig. 41, FAO-56
    p = max([0.1 p]); %Fig. 41, FAO-56
    RAW(i,1) = p*TAW(i); %Eq. 83, FAO-56, readily available water (mm)
    if Dr(i-1) > RAW(i)
